...
## v0.18 speed/cache pipeline

AI応答高速化のため、以下の流れに変更しています。

1. Google連携/再取得時にGoogleカレンダーを取得し、FlutterキャッシュとSupabase `schedule_items` に同期
2. AI対話時は毎回Googleカレンダーを取得せず、Flutterキャッシュ済み予定をFastAPIへ送信
3. 最初のGemini呼び出しをRouterとして使い、pgvector類似記憶検索が必要か判定
4. 必要な時だけpgvector検索を最大3件実行
5. OR-Toolsは必ず実行し、予定候補を厳密配置
6. Geminiで説明文を生成してすぐ画面へ返す
7. 会話ログ・提案・記憶・カレンダー同期のSupabase保存はBackgroundTasksで裏側実行
8. 追加/変更/削除をGoogleカレンダーに反映した直後、FlutterキャッシュとSupabase `schedule_items` も同時更新

このため、通常のAI対話ではGoogle Calendar APIの毎回取得を避け、必要時だけ再取得します。


## v0.20 Profile System

プロフィールタブを追加しました。

- 初回アンケートで `user_profiles` と `user_rules` を作成
- 自由記入欄でユーザー独自の生活習慣を保存
- 傾向分析ボタンでカレンダーから生活傾向を抽出
- `profile_review_items` でAIの仮説・質問・ルール変更候補を一緒に管理
- 「最近忙しいですか？」の回答を `current_user_state` に反映
- AI対話時は `user_profiles`、`usage=always` の `user_rules`、`current_user_state` を毎回読み込む

既存Supabaseには次を実行してください。

```bash
pbcopy < ~/Downloads/shift_agent_app/supabase/migrations/006_profile_system.sql
```

Supabase SQL Editorで貼り付けてRunします。
